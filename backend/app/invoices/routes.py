from flask import Blueprint, request, jsonify
from ..auth.decorators import require_auth
from .models import Invoice, InvoiceLineItem, InvoicePayment, INVOICE_STATUSES, PAYMENT_TERMS
from config.appwrite_setup import databases, DATABASE_ID
from appwrite.id import ID
from datetime import datetime

invoices_bp = Blueprint('invoices', __name__)

# Validate status transition
VALID_STATUS_TRANSITIONS = {
    'draft': ['sent'],
    'sent': ['accepted', 'declined', 'draft', 'overdue', 'draft'],
    'accepted': ['paid', 'partially_paid', 'overdue', 'voided', 'draft'],
    'declined': ['draft', 'voided', 'draft'],
    'paid': ['voided', 'draft'],
    'partially_paid': ['paid', 'overdue', 'voided', 'draft'],
    'overdue': ['paid', 'partially_paid', 'voided', 'draft'],
    'voided': ['draft']
}

@invoices_bp.route('/create_invoice', methods=['POST'])
@require_auth
def create_invoice():
    """Create a new invoice"""
    try:
        data = request.json
        if not data:
            return jsonify({'error': 'No data provided'}), 400
            
        user_id = request.headers.get('X-Appwrite-User-ID')
        
        # Validate required fields
        required_fields = ['customer_id', 'line_items', 'payment_terms']
        missing_fields = [field for field in required_fields if field not in data]
        if missing_fields:
            return jsonify({
                'error': f'Missing required fields: {", ".join(missing_fields)}'
            }), 400
            
        # Validate customer exists and belongs to user
        if not Invoice.validate_customer(user_id, data['customer_id']):
            return jsonify({
                'error': f'Customer {data["customer_id"]} not found or does not belong to user {user_id}'
            }), 400
            
        # Validate line items
        if not isinstance(data['line_items'], list) or not data['line_items']:
            return jsonify({'error': 'At least one line item is required'}), 400
            
        # Validate payment terms
        if data['payment_terms'] not in PAYMENT_TERMS:
            return jsonify({
                'error': f'Invalid payment terms. Must be one of: {", ".join(PAYMENT_TERMS)}'
            }), 400
            
        # Create and validate line items
        line_items = []
        for item_data in data['line_items']:
            try:
                # Infer type from item_id
                if 'item_id' not in item_data:
                    return jsonify({'error': 'Line item missing item_id field'}), 400
                    
                item_id = item_data['item_id']
                if item_id.startswith('PROD'):
                    item_data['type'] = 'product'
                    # Validate product exists and belongs to user
                    if not Invoice.validate_product(user_id, item_id):
                        return jsonify({
                            'error': f'Product {item_id} not found or does not belong to user {user_id}'
                        }), 400
                elif item_id.startswith('SERV'):
                    item_data['type'] = 'service'
                    # Validate service exists and belongs to user
                    if not Invoice.validate_service(user_id, item_id):
                        return jsonify({
                            'error': f'Service {item_id} not found or does not belong to user {user_id}'
                        }), 400
                else:
                    return jsonify({'error': f'Invalid item_id format: {item_id}. Must start with PROD or SERV'}), 400
                        
                line_items.append(InvoiceLineItem.from_dict(item_data))
            except Exception as e:
                return jsonify({'error': f'Invalid line item data: {str(e)}'}), 400
            
        # Create invoice object with line items
        invoice = Invoice.create_invoice(
            user_id=user_id,
            customer_id=data['customer_id'],
            line_items=line_items,
            payment_terms=data['payment_terms'],
            notes=data.get('notes')
        )
        
        # Create the invoice document
        invoice_doc = databases.create_document(
            database_id=DATABASE_ID,
            collection_id='invoices',
            document_id=ID.unique(),
            data=invoice.to_dict(for_appwrite=True)
        )
        
        return jsonify({
            "message": "Invoice created successfully", 
            "invoice": invoice_doc
        }), 201
        
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@invoices_bp.route('/get_invoice/<invoice_id>', methods=['GET'])
@require_auth
def get_invoice(invoice_id: str):
    """Get a specific invoice by invoice ID"""
    try:
        user_id = request.headers.get('X-Appwrite-User-ID')
        invoice = Invoice.get_invoice_by_id(invoice_id, user_id)
        return jsonify(invoice.to_dict()), 200
        
    except Exception as e:
        return jsonify({"error": str(e)}), 404

@invoices_bp.route('/list_invoices', methods=['GET'])
@require_auth
def list_invoices():
    """List all invoices for the current user"""
    try:
        user_id = request.headers.get('X-Appwrite-User-ID')
        invoices = Invoice.get_invoices_by_user(user_id)
        return jsonify([inv.to_dict() for inv in invoices]), 200
        
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@invoices_bp.route('/update_invoice/<invoice_id>', methods=['PATCH'])
@require_auth
def update_invoice(invoice_id):
    """Update an invoice by invoice ID"""
    try:
        data = request.json
        if not data:
            return jsonify({'error': 'No data provided'}), 400
            
        user_id = request.headers.get('X-Appwrite-User-ID')
        
        # Get the invoice
        invoice = Invoice.get_invoice_by_id(invoice_id, user_id)
        if not invoice:
            return jsonify({'error': 'Invoice not found'}), 404
            
        # Handle status update if provided
        if 'status' in data:
            try:
                invoice.update_status(data['status'])
            except ValueError as e:
                return jsonify({'error': str(e)}), 400
                
        # Update other fields if provided
        if 'customer_id' in data:
            # Validate new customer exists and belongs to user
            if not Invoice.validate_customer(user_id, data['customer_id']):
                return jsonify({
                    'error': f'Customer {data["customer_id"]} not found or does not belong to user {user_id}'
                }), 400
            invoice.customer_id = data['customer_id']
            
        if 'payment_terms' in data:
            if data['payment_terms'] not in PAYMENT_TERMS:
                return jsonify({
                    'error': f'Invalid payment terms. Must be one of: {", ".join(PAYMENT_TERMS)}'
                }), 400
            invoice.payment_terms = data['payment_terms']
            
        if 'line_items' in data:
            try:
                line_items = []
                for item_data in data['line_items']:
                    # Infer type from item_id
                    if 'item_id' not in item_data:
                        return jsonify({'error': 'Line item missing item_id field'}), 400
                        
                    item_id = item_data['item_id']
                    if item_id.startswith('PROD'):
                        item_data['type'] = 'product'
                        # Validate product exists and belongs to user
                        if not Invoice.validate_product(user_id, item_id):
                            return jsonify({
                                'error': f'Product {item_id} not found or does not belong to user {user_id}'
                            }), 400
                    elif item_id.startswith('SERV'):
                        item_data['type'] = 'service'
                        # Validate service exists and belongs to user
                        if not Invoice.validate_service(user_id, item_id):
                            return jsonify({
                                'error': f'Service {item_id} not found or does not belong to user {user_id}'
                            }), 400
                    else:
                        return jsonify({'error': f'Invalid item_id format: {item_id}. Must start with PROD or SERV'}), 400
                            
                    line_items.append(InvoiceLineItem.from_dict(item_data))
                invoice.line_items = line_items
            except (KeyError, ValueError) as e:
                return jsonify({'error': f'Invalid line item data: {str(e)}'}), 400
                
        # Recalculate total
        invoice._calculate_total()
        
        # Update in database
        db = Invoice.get_database()
        if not invoice.document_id:
            return jsonify({'error': 'Document ID not found'}), 500
            
        # Convert to Appwrite format and update
        appwrite_data = invoice.to_dict(for_appwrite=True)
        result = db.update_document(
            database_id=DATABASE_ID,
            collection_id='invoices',
            document_id=invoice.document_id,
            data=appwrite_data
        )
        
        # Create response with updated invoice
        updated_invoice = Invoice.from_dict(result, document_id=result['$id'])
        
        return jsonify({
            'message': 'Invoice updated successfully',
            'invoice': updated_invoice.to_dict()
        }), 200
            
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@invoices_bp.route('/update_status/<invoice_id>', methods=['POST'])
@require_auth
def update_invoice_status(invoice_id: str):
    """Update invoice status"""
    try:
        data = request.json
        if not data or 'status' not in data:
            return jsonify({'error': 'Status is required'}), 400
            
        user_id = request.headers.get('X-Appwrite-User-ID')
        new_status = data['status']
        
        # Get current invoice
        invoice = Invoice.get_invoice_by_id(invoice_id, user_id)
        if not invoice:
            return jsonify({'error': 'Invoice not found'}), 404
        
        # Validate status transition
        if new_status not in VALID_STATUS_TRANSITIONS.get(invoice.status, []):
            return jsonify({
                'error': f'Invalid status transition from {invoice.status} to {new_status}'
            }), 400

        # Update status using the model's method
        try:
            invoice.update_status(new_status, data.get('reason'))
        except ValueError as e:
            return jsonify({'error': str(e)}), 400
            
        # Update the document
        databases.update_document(
            database_id=DATABASE_ID,
            collection_id='invoices',
            document_id=invoice.document_id,
            data=invoice.to_dict(for_appwrite=True)
        )
        
        return jsonify({'message': 'Status updated successfully'}), 200
            
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@invoices_bp.route('/add_payment/<invoice_id>', methods=['POST'])
@require_auth
def add_payment(invoice_id):
    """Add a payment to an invoice"""
    try:
        data = request.json
        if not data:
            return jsonify({'error': 'No data provided'}), 400
            
        user_id = request.headers.get('X-Appwrite-User-ID')
        
        # Get the invoice
        invoice = Invoice.get_invoice_by_id(invoice_id, user_id)
        if not invoice:
            return jsonify({'error': 'Invoice not found'}), 404
            
        # Validate required fields
        required_fields = ['amount', 'payment_method']
        missing_fields = [field for field in required_fields if field not in data]
        if missing_fields:
            return jsonify({
                'error': f'Missing required fields: {", ".join(missing_fields)}'
            }), 400
            
        # Create payment object
        try:
            payment = InvoicePayment(
                amount=float(data['amount']),
                payment_method=data['payment_method'],
                notes=data.get('notes'),
                check_number=data.get('check_number')
            )
        except ValueError as e:
            return jsonify({'error': f'Invalid payment data: {str(e)}'}), 400
            
        # Add payment to invoice
        try:
            invoice.add_payment(payment)
        except ValueError as e:
            return jsonify({'error': str(e)}), 400
            
        # Update invoice in database
        db = Invoice.get_database()
        result = db.update_document(
            database_id=DATABASE_ID,
            collection_id='invoices',
            document_id=invoice.document_id,
            data=invoice.to_dict(for_appwrite=True)
        )
        
        # Create response with updated invoice
        updated_invoice = Invoice.from_dict(result, document_id=result['$id'])
        
        return jsonify({
            'message': 'Payment added successfully',
            'invoice': updated_invoice.to_dict(),
            'payment': payment.to_dict()
        }), 200
            
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@invoices_bp.route('/get_payments/<invoice_id>', methods=['GET'])
@require_auth
def get_payments(invoice_id: str):
    """Get all payments for an invoice"""
    try:
        user_id = request.headers.get('X-Appwrite-User-ID')
        
        # Get current invoice
        invoice = Invoice.get_invoice_by_id(invoice_id, user_id)
        if not invoice:
            return jsonify({'error': 'Invoice not found'}), 404
            
        return jsonify({
            'payments': [payment.to_dict() for payment in invoice.payments],
            'total_paid': invoice.amount_paid,
            'balance': invoice.balance
        }), 200
            
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@invoices_bp.route('/get_payment/<invoice_id>/<reference_id>', methods=['GET'])
@require_auth
def get_payment(invoice_id: str, reference_id: str):
    """Get a specific payment by reference ID"""
    try:
        user_id = request.headers.get('X-Appwrite-User-ID')
        
        # Get current invoice
        invoice = Invoice.get_invoice_by_id(invoice_id, user_id)
        if not invoice:
            return jsonify({'error': 'Invoice not found'}), 404
            
        # Find payment with matching reference_id
        payment = next(
            (p for p in invoice.payments if p.reference_id == reference_id),
            None
        )
        if not payment:
            return jsonify({'error': 'Payment not found'}), 404
            
        return jsonify({
            'payment': payment.to_dict()
        }), 200
            
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@invoices_bp.route('/delete_invoice/<invoice_id>', methods=['DELETE'])
@require_auth
def delete_invoice(invoice_id: str):
    """Delete an invoice by invoice ID"""
    try:
        user_id = request.headers.get('X-Appwrite-User-ID')
        Invoice.delete_invoice(invoice_id, user_id)
        return jsonify({"message": "Invoice deleted successfully"}), 200
        
    except Exception as e:
        return jsonify({"error": str(e)}), 400
