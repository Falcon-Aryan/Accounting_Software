    ...
    try:
        ...
        with open(file_path, 'r') as f:
            ...
            accounts_map = {acc['id']: acc['name']
                            for acc in chart_data.get('accounts', [])}
        ...
        if not transaction_data.get('entries'):
            ...
            if product_type == 'inventory_item':
                ...
                with open(os.path.join(DATA_DIR, uid, 'products.json'), 'r') as f:
                    ...
                    products_map = {
                        p['id']: p for p in products_data.get('products', [])}
...
def validate_transaction_accounts(transaction_type: str, sub_type: str, entries: List[Dict]) -> tuple[bool, str]:
    ...
    try:
        ...
        with open(file_path, 'r') as f:
            chart_data = json.load(f)
            accounts = {
                acc['id']: acc for acc in chart_data.get('accounts', [])}
        ...
        valid_account_types = get_valid_account_types(
...
def get_valid_account_types(transaction_type: str, sub_type: str) -> List[str]:
    ...
    asset_accounts = ['Bank', 'Other Current Asset',
                      'Fixed Asset', 'Accounts Receivable']
    liability_accounts = ['Credit Card', 'Accounts Payable',
    ...
    return list(set(valid_types.get(transaction_type, [])))
    ...
    try:
        ...
        for entry in transaction.entries:
            ...
            if normal_balance == 'debit':
                ...
                # Debit increases the balance, Credit decreases it
                if entry.type == 'debit':
                    account['currentBalance'] = account.get(
                        'currentBalance', 0) - entry.amount
                else:  # credit
                    account['currentBalance'] = account.get(
                        'currentBalance', 0) + entry.amount
            else:  # credit normal
...
def validate_transaction():
    ...
    try:
        ...
        total_debits = sum(
            float(entry['amount']) for entry in data['entries'] if entry['type'] == 'debit')
        total_credits = sum(
            float(entry['amount']) for entry in data['entries'] if entry['type'] == 'credit')
...
def list_transactions():
    ...
    try:
        ...
        if reference_type:
            transactions = [
                t for t in transactions if t['reference_type'] == reference_type]

        reference_id = request.args.get('reference_id')
        if reference_id:
            transactions = [
                t for t in transactions if t['reference_id'] == reference_id]
...
def patch_transaction(transaction_id):
    ...
    try:
        ...
        current_transaction['updated_at'] = datetime.utcnow(
        ).date().isoformat()
...
def void_transaction(transaction_id):
    ...
    try:
        ...
        if transaction['reference_type'] == 'invoice_payment':
            invoice_id = transaction['reference_id'].split(
                '_')[0]  # Get invoice ID from reference
            ...
            # Find invoice
            invoice = next((inv for inv in invoices_data.get(
                'invoices', []) if inv['id'] == invoice_id), None)
            if invoice:
                # Recalculate total paid
                total_paid = sum(
                    float(p.get('amount', 0))
                    for p in invoice.get('payments', [])
                    # Exclude voided payment
